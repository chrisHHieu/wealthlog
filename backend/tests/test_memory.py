...
async def test_save_user_fact_new(db: AsyncSession):
    ...
    from app.services.memory import save_user_fact
...
def _patch_get_session(db: AsyncSession):
    """Patch app.services.memory.get_session / app.mcp.db.get_session to use test DB.
    ...
    return patch("app.services.memory.get_session", _mock_get_session)
