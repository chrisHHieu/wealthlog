...
def test_compact_history_passthrough_when_short():
    ...
    from app.ai.agent.compaction import _compact_history
...
async def test_facts_from_session_a_appear_in_session_b_prompt(db: AsyncSession):
    ...
    from app.ai.memory.facts import build_facts_prompt, save_user_fact
...
def _patch_get_session(db: AsyncSession):
    ...
    stack = ExitStack()
    stack.enter_context(patch("app.ai.memory.facts.get_session", _mock))
    stack.enter_context(patch("app.ai.memory.episodic.get_session", _mock))
    return stack
