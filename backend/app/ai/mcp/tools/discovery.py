...
# \w includes underscore, so 'created_at' tokenizes as one whole identifier
# rather than leaking 'create' as a free-standing token. Critical: a substring
# check would block any SELECT touching `created_at`/`updated_at` columns.
...
def _is_read_only(sql: str) -> bool:
    ...
    if not tokens:
        return False
    first_word_match = _WORD_RE.match(normalized)
